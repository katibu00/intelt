<script type="text/javascript">
    $(document).ready(function() {
                            ...
                            $.each(res.regulars, function(key, regular) {
                                ...
                                html +=
                                    ...
                                    '<td class="text-center"><div class="form-check form-check-inline"><input class="form-check-input course" id="selected" type="checkbox" data-course_code="' +
                                        ...
                                        regular.id + '" data-semester="' +
                                        regular.semester + '"/></div></td>';
                            ...
                            $.each(res.electives, function(key, elective) {
                                ...
                                html +=
                                    ...
                                    '<td class="text-center"><div class="form-check form-check-inline"><input class="form-check-input course" id="selected" type="checkbox" data-course_code="' +
                                        ...
                                        elective.id + '" data-semester="' +
                                        elective.semester + '" /></div></td>';
                            ...
                            $.each(res.cos, function(key, co) {
                                ...
                                html +=
                                    ...
                                    '<td class="text-center"><div class="form-check form-check-inline"><input class="form-check-input course" id="selected" type="checkbox" data-course_code="' +
                                        ...
                                        co.course.id + '" data-semester="' +
                                        co.course.semester+ '" /></div></td>';
        ...
        $(document).on('click', '.course', function(e) {
            ...
            if ($(this).is(':checked')) {
                ...
                if(semester == 'first'){
                    ...
                    if($('.first-selected-courses-table').find('td').filter(function () {return this.innerHTML == course_code;}).length>0){ 
                    Command: toastr["error"]("Course Already Selected")
                            toastr.options = {
                            "closeButton": false,
                            "debug": false,
                            "newestOnTop": false,
                    ...
                    var row = '';
                    row +=
                    '<tr class="serial_first">' +
                    '<td class="key">' + first_count +'</td>' +
                    '<td>' + course_code + '</td>' +
                    '<td>' + course_title + '</td>' +
                    ...
                    '<tr>';
                    $('.first-selected-courses-table > tbody:last-child').append(row);
                    first_count++
                }
                if(semester == 'second'){

                    if($('.second-selected-courses-table').find('td').filter(function () {return this.innerHTML == course_code;}).length>0){ 
                    Command: toastr["error"]("Course Already Selected")
                            toastr.options = {
                            "closeButton": false,
                            "debug": false,
                    ...
                    var row = '';
                    row +=
                    '<tr class="serial_second">' +
                    '<td class="key">' + second_count +'</td>' +
                    '<td>' + course_code + '</td>' +
                    '<td>' + course_title + '</td>' +
                    '<td>' + credit_unit + '<input class="selected" type="hidden" name="course_id[]" value="'+course_id+'" />'+ '</td>' +
                    '<td class="text-center"><button class="DeleteButton" data-code="'+course_code+'" data-credits="'+credit_unit+'" >X</button></td>' +
                    '<tr>';
                    $('.second-selected-courses-table > tbody:last-child').append(row);
                    second_count++
                }
        ...
        //remove first semester courses
        $(".first-selected-courses-table").on("click", ".DeleteButton", function() {
            ...
            $(".serial_first").each(function(row_count){
        ...
        //remove second semester courses
        $(".second-selected-courses-table").on("click", ".DeleteButton", function() {
            let credits = $(this).data('credits');
            let code = $(this).data('code');
            totalcr -= credits;
            lineNo--;
            ...
            $(".serial_second").each(function(row_count){
                $(this).children(".key").html(row_count+1);
            });
        ...
        $(document).on('click', '.reset', function(e) {
            ...
            $(".first-selected-courses-table > tbody").empty();
            $(".second-selected-courses-table > tbody").empty();
            totalcr = 0;
            lineNo = 1;
            first_count = 1;
            second_count = 1;
